...
<Callout type="warn">
  While the library is SSR-safe, the Facebook SDK itself is a client-side script. Facebook widgets (Like, Share,
  Comments, etc.) render only on the client. During SSR, the component tree renders but the Facebook widget is not
  painted until the client hydrates and the SDK loads.
</Callout>
